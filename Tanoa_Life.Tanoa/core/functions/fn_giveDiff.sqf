...
if (_bool) then {
    if ([true,_item,(parseNumber _val)] call life_fnc_handleInv) then {
        hint format [localize "STR_MISC_TooMuch",_from getVariable ["RP_ID",name _from],_val,(localize _type)];
    };
} else {
    if ([true,_item,(parseNumber _val)] call life_fnc_handleInv) then {
        hint format [localize "STR_MISC_TooMuch_2",_from getVariable ["RP_ID",name _from],_val,(localize _type)];
    };
};