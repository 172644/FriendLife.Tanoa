...
if ((player getVariable "sit") == 1) exitWith{hint "Vous etes deja assis !"};
if (vehicle _unit != _unit) exitWith { hint "Vous etes assis ici deja !!" };
if (_unit distance _chair > 4) exitWith { hint "Vous etes trop loin !" };
...
if (typeOf _chair in ["equipement_hospital_bed","equipement_clinic_bed","equipement_xray_machine","equipement_cat_scan_machine"]) then {
	//_sitStyle = "Acts_LyingWounded_loop";
};
...
if ([str _chair,"bench"] call KRON_StrInStr) then
{
	...
	_zPos = _posATL select 2;
	
	_zPos = switch (typeOf _chair) do
	{
		case "tas_banquette_kfc": { _zPos - 1 };
		case "tas_banquette_double_kfc": { _zPos - 1 };
		case "tas_chaise_1_kfc": { _zPos - 0.5 };
		case "tas_chaise_kfc": { _zPos - 1 };
		case "Land_Magazine_rifle_F": { _zPos - 0.4 };
		default { _zPos };
	};
	
    _unit setPosATL ([_posATL select 0, _posATL select 1, _zPos]);
